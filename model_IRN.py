...
# 1. Get the data
data = pd.read_csv("./data/IRANdata.csv")
print(data.head())
...
# 4. Make model
clf_model_IRN = RandomForestClassifier()

# 5. Fit the model
clf_model_IRN.fit(X_train, y_train)
# print(clf_model.get_params())

# 6. Make prediction
y_pred = clf_model_IRN.predict(X_test)

# 7. Check Accuracy
acc = clf_model_IRN.score(X_test, y_pred)
print(f"Accuracy of RFC model is: {acc * 100:0.2f} %")

# 8. Save the model
# save the model
# filename = 'clfmodel_IRN.pkl'
# pickle.dump(clf_model_IRN, open(filename, 'wb'))

# 9. Load model
try:
    print("Model is loading ... ")
    time.sleep(2)
    load_model = pickle.load(open("./models/clfmodel_IRN.pkl",'rb'))
    print("clfmodel_IRN is loaded successfuly.")
except:
...
class_App_names = ['Snappfood', 'Jimomarket', 'Digikala']
...
print(f"In IRAN, for a {predicted_Gender_class}, with education level of {predicted_Education_class}, age of {Age}, with online experience of {Exp_online} year/years, and shopping experience from online Apps {Exp_app} year/years, It seems that the prefetable Grocery App is : {predicted_App_class}.")
